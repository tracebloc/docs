...
- **Encryption in transit.** All client ↔ platform traffic is TLS, on an **outbound-only** connection.
...
The client needs outbound HTTPS to: `*.docker.io`, `ghcr.io`, `raw.githubusercontent.com`, `*.github.io`, `*.tracebloc.io`, and `pypi.org`. Nothing needs to reach *in*.
