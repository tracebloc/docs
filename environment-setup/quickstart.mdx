...
A running workspace in about 10 minutes, with one command. Deploying on a specific environment instead (EKS, AKS, bare-metal, OpenShift)? See [Deployment environments](/environment-setup/setup-guide).

## Before you start

| You need | Minimum |
|---|---|
| A machine | macOS, Linux, or Windows · 4 CPU · 8 GB RAM · 20 GB free disk |
...
<Steps>
  ...
  <Step title="Run the installer">
    ...
    <Info>
      **▶ Install demo** — a ~90-second terminal recording goes here, showing the one-liner run end to end: prompts → setup steps → summary → the client turning **Online**. *(placeholder)*
    </Info>
  ...
  <Step title="Verify it's connected">
    ...
    Look for the tracebloc pods (`mysql-client`, `…-jobs-manager`, `…-requests-proxy`) in `Running` state. Then open your [clients page](https://ai.tracebloc.io/clients) — the client should show **Online**.
  </Step>
</Steps>
