...
```python
# Turn built-in imputation off so your model owns missing-data handling
training_plan.handle_missing_values(False)

# Or pick the imputation strategy: median (default), mean, most_frequent,
# constant, iterative (MICE), or 'none'
training_plan.imputation_strategy('iterative')

# Categorical encoding: label (default) or onehot
training_plan.encoding_strategy('onehot')

# Numeric feature scaling on/off + scaler choice (tabular & time-to-event)
training_plan.normalize_features(True)
training_plan.scaler('RobustScaler')

# Time-series classification only: opt-in missingness-indicator channels —
# a 0/1 <column>__missing feature per numeric column carrying the raw NaN
# mask. The model's input width grows, identically at every edge.
